  ...
  <Layout>
    ...
    <br>
      <div class="bg-white grid v-screen 	 justify-items-center relative  shadow-md rounded-lg xl:h-[80] md:h-[80]  sm:h-[680px]   ">
          ...
          <DataTable :data="categorias.data" :columns="columns1" 
     ...
     <thead class="">
            <tr class="bg-gray-800 text-white">
              <th scope="col" class="px-10 py-2" v-text="`#`" />              
              <th class="px-10 py-4">Raza</th>
...
const categorias = ref(props.paginationObject)
...
{data:'description'},
  ...
  { data: null, orderable: false, render: function (data) { 
    ...
    return '<button class="rounded-xl text-red-600 font-semibold text-xl	  hover:text-gray-800/50 w-1/4" data-id="' + data.name +','+ data.id +'" id="edit"> <i class="fa-solid fa-trash"></i></button>'; }, },
]

$(document).on("click","#edit", function(){

var partes = $(this).data('id').split(',');

    Swal.fire({
    title: '¿Desea eliminar la raza '+partes[0]+'? ',
    icon: 'question',
    ...
    if (result.isConfirmed) {
      Swal.fire(
        ...
        'success'
           ...
           window.location.href = "razas/"+partes[1]+"";
    }
  })
